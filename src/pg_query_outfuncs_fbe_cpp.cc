...
#define WRITE_LIST_FIELD(outname, outname_json, fldname) \
    if (node->fldname != NULL) {                         \
        ...
        foreach (lc, node->fldname) {                    \
            ...
            _outNode(out->outname.back(), lfirst(lc));   \
        }                                                \
    }
...
#define WRITE_SPECIFIC_NODE_FIELD(typename, typename_underscore, outname, outname_json, fldname) \
    { _out##typename(&out->outname, &node->fldname); }

#define WRITE_SPECIFIC_NODE_PTR_FIELD(typename, typename_underscore, outname, outname_json, fldname) \
    if (node->fldname != NULL) {                                                                     \
        out->outname = (new pg_query::typename());                                                   \
        _out##typename(out->outname, node->fldname);                                                 \
    }

static void _outNode(pg_query::Node& out, const void* obj);

static void _outList(pg_query::List* out_node, const List* node) {
    const ListCell* lc;

    foreach (lc, node) {
        out_node->items.emplace_back();
        _outNode(out_node->items.back(), lfirst(lc));
    }
}

static void _outIntList(pg_query::IntList* out_node, const List* node) {
    const ListCell* lc;
...
static void _outOidList(pg_query::OidList* out_node, const List* node) {
    const ListCell* lc;
...
static void _outInteger(pg_query::Integer* out_node, const Value* node) { out_node->ival = (node->val.ival); }

static void _outFloat(pg_query::Float* out_node, const Value* node) { out_node->str = (node->val.str); }

static void _outString(pg_query::String* out_node, const Value* node) { out_node->str = (node->val.str); }

static void _outBitString(pg_query::BitString* out_node, const Value* node) { out_node->str = (node->val.str); }

static void _outNull(pg_query::Null* out_node, const Value* node) {
    // Null has no fields
}
...
pg_query::ParseResult pg_query_nodes_to_fbe(const void* obj) {
    ...
    foreach (lc, (List*)obj) {
        ...
        _outRawStmt(&(parse_result.stmts[size - 1]), (const RawStmt*)lfirst(lc));
    }
